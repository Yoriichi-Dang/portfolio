...
export const ToggleTheme = ({
  ...
  const toggleTheme = useCallback(async () => {
    ...
    switch (animationType) {
      ...
      case 'flip-x-in': {
        // Fix: Avoid lexical declaration in case, and prevent style leakage
        const css = `
        ...
        // Remove existing flip-x-in style if present
        const prev = document.getElementById('toggle-theme-flip-x-in-style');
        if (prev) prev.remove();

        // Inject new style with unique ID
        const styleEl = document.createElement('style');
        styleEl.id = 'toggle-theme-flip-x-in-style';
        styleEl.textContent = css;
        document.head.appendChild(styleEl);

        // Auto cleanup after animation is finished
        setTimeout(() => {
