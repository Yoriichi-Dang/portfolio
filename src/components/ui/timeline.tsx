...
export const Timeline = ({ heading, description, data }: Props) => {
  ...
  return (
    <div className="w-fullfont-sans md:px-1" ref={containerRef}>
      <div className="mx-auto mt-4 max-w-7xl px-4 md:px-8 lg:px-10">
        <h2 className="mb-4 max-w-4xl text-lg text-black md:text-4xl dark:text-white">{heading}</h2>
      ...
      <div ref={ref} className="relative mx-auto max-w-7xl pb-20">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-12 md:gap-10 md:pt-24">
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              <div className="absolute left-3 flex h-10 w-10 items-center justify-center rounded-full bg-white md:left-3 dark:bg-black">
                <div className="bg-primary h-4 w-4 rounded-full border border-neutral-300 p-2 dark:border-neutral-700" />
              </div>
              <h3 className="text-foreground hidden text-xl font-bold md:block md:pl-20 md:text-3xl">
                {item.title}
              </h3>
            </div>
