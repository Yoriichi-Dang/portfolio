...
export function ModeToggle() {
  ...
  return (
    ...
    >
      {theme === 'light' && <svg className="h-5 w-5">☀️</svg>}
      {theme === 'dark' && <svg className="h-5 w-5">🌙</svg>}
      {theme === 'system' && <svg className="h-5 w-5">💻</svg>}
    </button>
  );
}
```
...
```tsx
<div style={{ backgroundColor: 'var(--brand-blue)' }}>Custom color that adapts</div>
```
